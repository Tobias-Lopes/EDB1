...
public:
    ...
    std::optional<std::string> buscar(const std::string& chave) const 
    {
        ...
        for(size_t delta = 0; delta < capacidade; delta++){
            auto indice = (base + delta) % capacidade;

            auto elementoAtual = array[indice];

            if( elementoAtual.estado == Estado::LIVRE){
                return std::nullopt;
            } else if( elementoAtual.estado == Estado::OCUPADO && elementoAtual.chave == chave){
                return elementoAtual.valor;
            }
            //else = REMOVIDO
        }
        //percorri todo o array e não encontrei
        return std::nullopt;
    }
